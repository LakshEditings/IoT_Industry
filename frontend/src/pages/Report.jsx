  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <div style={{
        ...
        <div style={{
          display: 'grid',
          gridTemplateColumns: '2.5rem 1fr 4rem repeat(6, 1fr) 6.5rem',
          backgroundColor: '#213555',
          position: 'sticky', top: 0, zIndex: 2,
        }}>
          {['#', 'Parameter', 'Value', ...CHANNELS, 'Graph'].map(h => (
            <div key={h} style={{
          ...
          {parameters.map((param, idx) => {
            ...
            return (
              <div key={param.id} style={{
                ...
                {/* Graph — click to popup */}
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '0.25rem', cursor: 'pointer' }}
                  title="Click to view full graph">
                  <Spark
                    data={sparks[ci]}
                    onClick={() => setPopup({ paramName: param.name, chKey })}
                  />
                </div>
              </div>
            );
